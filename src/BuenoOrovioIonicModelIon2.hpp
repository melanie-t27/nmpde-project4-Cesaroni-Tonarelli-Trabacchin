...
public:
    ...
    double implicit_coefficient(const std::array<double, K_ion>& u, size_t /*valid_u*/, GatingVariables<N_ion>& vars) override {
        return (1 - this->H(u[0] - this->theta_w))/this->tau_o(u[0])  + vars.get(0) / this->tau_fi * this->H(u[0] - this->theta_v) * (u[0] - this->theta_v);
    }

    double explicit_coefficient(const std::array<double, K_ion>& u, size_t /*valid_u*/, GatingVariables<N_ion>& vars) override {
        return -this->u_u * vars.get(0) / this->tau_fi  * this->H(u[0] - this->theta_v) * (u[0] - this->theta_v) - 
        this->u_0 * (1 - this->H(u[0] - this->theta_w))/this->tau_o(u[0]) + this->H(u[0] - this->theta_w) / this->tau_so(u[0]) -
