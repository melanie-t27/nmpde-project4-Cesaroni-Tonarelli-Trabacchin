...
/*
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q; ++q) {
        for (unsigned int i = 0; i < dofs_per_cell; ++i) {
            for (unsigned int j = 0; j < dofs_per_cell; ++j) {
              cell_Z_matrix(i,j) += chi * C_m * getImplicitCoefficient(cell->active_cell_index(), q) * 
...
void FESolver::assemble_rhs(const double time) {
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()){
    ...
    for (unsigned int q = 0; q < n_q; ++q) {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i) {
        cell_rhs(i) += (theta * I_app_new_loc + (1.0 - theta) * I_app_old_loc -  chi * C_m * 85.7 * getExplicitCoefficient(cell->active_cell_index(), q) - C_m * chi * getImplicitCoefficient(cell->active_cell_index(), q) * 84.0) *
