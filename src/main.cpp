...
int main(int argc, char *argv[]){
    ...
    Solver<1,1,3> solver("../meshes/cuboid.msh", degree, T, deltat, theta_fe, theta_ode, ionic_model, coupler, std::move(d), std::move(I_app), std::move(u_0), gating_variables_0);
    solver.solve();
    return 0;
}